...
const musicSelectionPanel = document.getElementById('musicSelectionPanel');
const saveMusicPanelBtn = document.getElementById('saveMusicPanelBtn');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Panel controls
    musicBtn.addEventListener('click', () => musicSelectionPanel.classList.remove('hidden'));
    settingsBtn.addEventListener('click', () => settingsPanel.classList.remove('hidden'));
    saveMusicPanelBtn.addEventListener('click', () => musicSelectionPanel.classList.add('hidden'));
...
function updateRoundIndicators() {
    ...
    const fillCount = sessionCount % 4;
...
function switchPhase() {
    chrome.storage.local.get(['autoStart', 'dingEnabled'], ({ autoStart = true, dingEnabled = true }) => {
        ...
        if (currentPhase === 'focus') {
            ...
            chrome.storage.local.set({ sessionCount });
            updateRoundIndicators();
            nextPhase = (sessionCount % 4 === 0) ? 'longBreak' : 'shortBreak';
        } else {
            nextPhase = 'focus';
        }
